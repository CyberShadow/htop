...
static bool LinuxProcessList_recurseProcTree(LinuxProcessList* this, const char* dirname, pid_t mainProcess, LinuxProcessScanData* psd) {
   ...
   for (struct dirent* entry; (entry = readdir(dir)) != NULL; ) {
      ...
      psd->name = name;
      psd->dirname = dirname;
      Process* proc = ProcessList_scanProcess(pl, pid, (ProcessScanData*) psd);
      if (proc) {
         proc->tgid = mainProcess ? mainProcess : pid;
      }
   }
   closedir(dir);
   return true;
...
void ProcessList_goThroughEntries(ProcessList* super) {
   ...
   psd.nowMsec = tv.tv_sec * 1000LL + tv.tv_usec / 1000LL;

   LinuxProcessList_recurseProcTree(this, PROCDIR, 0, &psd);
}
